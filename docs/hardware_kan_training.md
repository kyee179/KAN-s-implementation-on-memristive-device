...
## Complete Physical Follow-up

The first training prototype uses a hardware-aware abstraction. For the stricter complete physical path, where `x^3` and `x^5` are generated by Gilbert multiplier cascades and SET/RESET updates follow DynamicMemdiode state evolution, see `docs/complete_physical_simulation.md`.
